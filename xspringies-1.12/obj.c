...
void restore_state()
{
	...
	masses = (mass *)xmalloc(sizeof(mass) * num_mass_alloc);
	memcpy(masses, masses_save, sizeof(mass) * num_mass_alloc);
	springs = (spring *)xmalloc(sizeof(spring) * num_spring_alloc);
	memcpy(springs, springs_save, sizeof(spring) * num_spring_alloc);

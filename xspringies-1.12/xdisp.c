	...
	switch (mode) {
	  ...
	  case M_SPRING:
		...
		if ((static_spring || action == -1 || cur_button == Button1) && selection >= 0 && is_mass && selection != start_sel) {
		    ...
		    if (!cur_shift)
		      unselect_all();
		    springs[newsel].status |= S_SELECTED;
		}
...
char *displayname, *geometry, *fgcolor, *bgcolor, *hlcolor;
{
	...
	if (mask = XParseGeometry(geometry, &(hints.x), &(hints.y),
	...
	XGetGeometry(dpy, main_win, &tmp_win, &xpos, &ypos,
		     (unsigned int *)&main_wid,
		     (unsigned int *)&main_ht, &bw, &dep);
    }
...
static void make_widgets()
{
	...
	add_modebutton(acts_pm, acts_win, 4, y+24, 4+coll_width+4, y+24+coll_height+4,"",
		       coll_bits, coll_width, coll_height,
		       M_COLLIDE, &mst.collide, TRUE);

	XDrawRectangle(dpy, acts_pm, drawgc, 16+xm+xoff*2, y+24, 64-xoff*2, 30);
	add_checkbox(acts_pm, acts_win, xm+40+xoff, y+18, xm+56+xoff, y+33, "", C_WTOP, &(mst.w_top));
	add_checkbox(acts_pm, acts_win, xm+8+xoff*2, y+33, xm+24+xoff*2, y+48, "Walls", C_WLEFT, &(mst.w_left));
	add_checkbox(acts_pm, acts_win, xm+72, y+33, xm+88, y+48, "", C_WRIGHT, &(mst.w_right));
	add_checkbox(acts_pm, acts_win, xm+40+xoff, y+48, xm+56+xoff, y+63, "", C_WBOTTOM, &(mst.w_bottom));
    }

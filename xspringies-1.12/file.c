...
FILE *read_file(char *file, boolean *p)
{
    FILE *f;
    char buff[MAXPATH];
    struct stat sbuf;
...
#ifdef COMPRESS
    sprintf(buff, "%s %s", COMPRESS, file);
...
int command;
{
    ...
    boolean selectnew = FALSE, pipe_file = FALSE;
    ...
    switch (command) {
      case F_INSERT:
      case F_LOAD:	
	if ((f = read_file(extend_file(file), &pipe_file)) == NULL) {
	    return FALSE;
	}
	...
	if (fgets(cmd, 5, f) != NULL && IS_CMD(MAGIC_CMD)) {
		...
		if (IS_CMD("mass")) {
			...
			fscanf(f, "%d", &temp);
			activate_mbutton(&(mst.collide), temp);
			skip_to_eol(f);
		    }
		} else if (IS_CMD("visc")) {
		    fscanf(f, "%lf\n", &(mst.cur_visc));
	...
	compress_file(extend_file(file));
	break;
    }
